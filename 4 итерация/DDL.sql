...
CREATE TABLE IF NOT EXISTS anime (
    id_name SERIAL,
    name_anime VARCHAR(256) UNIQUE NOT NULL PRIMARY KEY,
...
CREATE TABLE IF NOT EXISTS users (
    id_users SERIAL,
    login VARCHAR(256) UNIQUE NOT NULL PRIMARY KEY,
...
CREATE TABLE IF NOT EXISTS voice_over_studio (
    id_studio SERIAL ,
    name_studio VARCHAR(256) UNIQUE NOT NULL PRIMARY KEY
);
...
CREATE TABLE IF NOT EXISTS genre (
    id_genre SERIAL ,
    name_genre VARCHAR(256) UNIQUE NOT NULL PRIMARY KEY,
...
CREATE TABLE IF NOT EXISTS rating_reviews (
    ...
    login VARCHAR(256) REFERENCES users(login) ON DELETE CASCADE,
    name_anime VARCHAR(256) REFERENCES anime(name_anime) ON DELETE CASCADE,
    rating INTEGER DEFAULT 10 CHECK (rating >= 0 AND rating <= 10),
